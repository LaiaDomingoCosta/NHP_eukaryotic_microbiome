---
title: "Eukdetect prevalence study (genus)"
...
```{r}
metadata = read.csv("DATA/Metadata_concat_final_euk.tsv", header = T, sep='\t', na.strings=c("","NA"))
track_table = read.csv("DATA/concat_track_table.tsv", header = T, sep='\t', na.strings=c("","NA"))
presence_absence_genus_table = read_delim("DATA/euk_presence_absence_genus_table.tsv", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
taxonomy_genus_table = read_delim("DATA/euk_taxonomy_genus_table.tsv", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
taxonomy_sp_table = read_delim("DATA/euk_taxonomy_species_table.tsv", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
```
...
There are no genus taxid nulls.
...
presence_absence_genus_table_f1_ind = merge(metadata[,c("sample_name","individual_id")],presence_absence_genus_table_f1,by.x="sample_name",by.y="Sample")
#Drop sample name
presence_absence_genus_table_f1_ind = presence_absence_genus_table_f1_ind[,!(names(presence_absence_genus_table_f1_ind) %in% c("sample_name"))]
#Sum values by individual
appearences_by_ind_genus_table_f1 = presence_absence_genus_table_f1_ind %>% group_by(individual_id) %>% summarise(across(everything(), sum), .groups = 'drop') %>% as.data.frame()
#Get presence-absence table
##Make copy to work on
presence_absence_by_ind_genus_table_f1 = appearences_by_ind_genus_table_f1
##Update rownames
rownames(presence_absence_by_ind_genus_table_f1)=presence_absence_by_ind_genus_table_f1$individual_id
...
presence_absence_by_ind_genus_table_f1_comb = merge(metadata_ind_interest_f1[,c("individual_id","comb")],presence_absence_by_ind_genus_table_f1,by.y = 0, by.x = "individual_id")
...
continent_colors=colorRampPalette(brewer.pal(3, name="Paired"))(length(leyend_uniq_continents))
...
pgroups_colors=colorRampPalette(brewer.pal(3, name="Paired"))(length(leyend_uniq_pgroups))
...
##Include annotations
lateral_info <-ComplexHeatmap::rowAnnotation(Continent=continent_info, Lifestyle=lifestyle_info, 
...
```{r}
ggsave(gg_htA1,filename = "./Graficas/Eukdetect_prevalence_genus_global.svg",height = 20,width =20)
...
```{r}
ggsave(gg_htA2,filename = "./Graficas/Eukdetect_prevalence_genus_species.svg",height = 20,width =20)
...
```{r}
ggsave(gg_htA3,filename = "./Graficas/Eukdetect_prevalence_genus_lifestyle.svg",height = 20,width =20)
...
```{r}
ggsave(gg_htA4,filename = "./Graficas/Eukdetect_prevalence_genus_continent.svg",height = 20,width =20)
...
```{r}
ggsave(gg_htA5,filename = "./Graficas/Eukdetect_prevalence_genus_country.svg",height = 20,width =20)
