...
def check_number_PAIRED_files(multiqc_table, r1_files_pattern, r2_files_pattern):
    """
    This function checks if the number of R1 and R2 PAIRED files matches.
    ...
    ------
    Exception
        If the number of files is not the equal raises an exception.
    ...
    """
    #Count R1 and R2 appearences
    r1_appearances = multiqc_table['Filename'].str.contains(r1_files_pattern).sum()
    r2_appearances = multiqc_table['Filename'].str.contains(r2_files_pattern).sum()
    #If the number of files does not match raise exception
    if r1_appearances != r2_appearances:
        raise Exception('Error! The number of R1 and R2 PAIRED files does not match!\nCheck your PAIRED files!')


def process_multiqc_counts(multiqc_table, fastq_pattern, r1_files_pattern, r2_files_pattern, sep, n_sep, col_name, pair_treatment):
    """
    ...
    pair_treatment : str
        Type of paired files treatment single(S) or doble(D).
    ...
    #if pair_treatment doble, multiply 2X R1 read count
    if pair_treatment == 'D':
        #Multiply 2X only the R1 file samples
        r1_sample_end = r1_files_pattern.replace(fastq_pattern, '')
        stats_counts.loc[stats_counts.Sample.str.endswith(r1_sample_end),'Total Sequences':]*=2    
...
def main():
    ...
    parser.add_argument(
            ...
            choices = ['S','D'],
            required = False,
            help = 'Paired Treatment (Optional) [Default:"S"]. Indicate how to treat PAIRED files counts, to count only once(S) or twice(D).'
    )
    ...
    paired_treatment = args.paired_treatment
    ...
    try:
        ...
        #Check that number of PAIRED files is equal
        check_number_PAIRED_files(stats_raw, r1_files_pattern, r2_files_pattern)
        ...
        for file_path in processed_path_files:
            print(file_path)
            #Read temp file
            temp_stat_file = pd.read_csv(file_path, sep = '\t')
            #Check that number of PAIRED files is equal
            check_number_PAIRED_files(temp_stat_file, r1_files_pattern, r2_files_pattern)
            #Save in stats list of df
            stats_processed_steps.append(temp_stat_file)
        ...
        stats_raw_counts = process_multiqc_counts(stats_raw, fastq_pattern, r1_files_pattern, r2_files_pattern, sample_name_separator, sep_appereance, 'input_raw_reads', paired_treatment)
        
        ##Get samples counts for different qc steps
        stats_process_counts_list = []
        for i in range(len(stats_processed_steps)):
            temp = process_multiqc_counts(stats_processed_steps[i], fastq_pattern, r1_files_pattern, r2_files_pattern, sample_name_separator, sep_appereance, processed_names[i], paired_treatment)
        ...
        #Prepare output files names sufixes
        counts_table_out_sufix = 'counts_track_table_pt' + paired_treatment + '.tsv'
        percent_table_out_sufix = 'percentages_track_table_pt' + paired_treatment + '.tsv'
        stats_table_out_sufix = 'qc_stats_table_pt' + paired_treatment + '.xlsx'
        ...
        if filter_mode == True:
            ##Update output files names sufixes
            counts_table_out_sufix = 'filtered_' + counts_table_out_sufix
            percent_table_out_sufix = 'filtered_' + percent_table_out_sufix
            stats_table_out_sufix = 'filtered_' + stats_table_out_sufix
            ...
            ##NOTE2: We add the fastq_pattern to try to build the fastq name (in some SE cases, there will not be a separator and this is the way that I came up with to take into account this cases)  
        ...
        ##Save final file
        ###Get final file name
        out_counts_table = treat_output_file_prefix_name(output_name_prefix, counts_table_out_sufix)
        ...
        ##Save final file
        ###Get final file name
        out_percentages_table = treat_output_file_prefix_name(output_name_prefix, percent_table_out_sufix)
        ###Treat output_directory parameter / Get full output file path
        ...
        ###Get final file name
        out_stats_table = treat_output_file_prefix_name(output_name_prefix, stats_table_out_sufix)
        ###Treat output_directory parameter / Get full output file path
